  ...
  const renderHelp = () => (
    <div className="akerun-screen akerun-modal-screen">
      <div className="akerun-modal-card akerun-help-card">
        ...
        <ol>
          <li>
            各指示の方向と「何回目に止めるか」を読み、外側のホイールから順に整列します。1回の通過は、指定方向へ回して正規ゲートを拾い、その位置で止めることです。途中で通り過ぎただけでは進みません。
          </li>
          <li>
        ...
        <h3>反応の読み方</h3>
        <p>
          正規ゲートはフライ接続と段階の進行を伴います。ゲート縁は近づいた合図、偽ゲートとその縁は浅い接触だけで段階が進みません。後半の作業台では針が現在位置、針の下の細い表示が保持進行です。OBSERVEではテンションとフェンスの安定帯、ボルトとハンドルの成功閾値も表示します。Pelagicでは止めた後の反応が変わるので、回転を止めてから確認します。
        </p>
        <h3>失敗とスコア</h3>
        <p>
