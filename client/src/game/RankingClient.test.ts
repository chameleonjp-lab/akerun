...
describe("RankingClient", () => {
  it("uses the verified contract and deduplicates a run token", async () => {
    const fetch = vi.fn(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        ...
        expect(body.operationTrace).toEqual({
          version: 1,
          events: [[0, "rotate", 1]],
          truncated: false,
        });
        return new Response(
          JSON.stringify({ accepted: true, score: result.score }),
          { status: 200 }
        );
      }
    );
    ...
    await expect(
      client.submit("player one", result, "run-token-1")
  ...
  it("sends a replay token only when retrying the same completed problem", async () => {
    ...
    await expect(
      client.prepareOfficialRun("player one", "AKERUN-01-V1", "completed-run-1")
  ...
  it("retries an interrupted begin request with the same token", async () => {
    ...
    await expect(client.beginOfficialRun("run-token-1")).resolves.toMatchObject(
      { status: "ok", problemId: "AKERUN-01-V1" }
    );
  ...
  it("abandons an unclaimed verified run after startup falls back locally", async () => {
    ...
    await expect(client.abandonOfficialRun("run-token-orphan")).resolves.toBe(
      true
    );
    expect(fetch).toHaveBeenCalledTimes(1);
  });
  ...
  it("normalizes legacy saved results before v2 submission", async () => {
    ...
    await expect(
      client.submit("player one", legacy, "run-token-legacy")
    ).resolves.toMatchObject({ accepted: true });
  });
  ...
  it("renders malformed remote ranking fields as safe fallback values", () => {
    expect(RankingClient.displayName({ playerName: "   \n  " })).toBe("匿名");
    expect(
      RankingClient.displayName({ playerName: "   ", display_name: " backup " })
    ).toBe("backup");
    expect(
      RankingClient.displayName({ display_name: "  player   one  " })
    ).toBe("player one");
    expect(RankingClient.score({ score: "not-a-number" })).toBe(0);
    expect(
      RankingClient.score({ score: "not-a-number", best_score: 120 })
    ).toBe(120);
    expect(RankingClient.score({ score: -4.9 })).toBe(0);
    expect(RankingClient.rank({ rank: "not-a-rank" }, 3)).toBe(3);
    expect(RankingClient.rank({ rank: 0 }, 3)).toBe(3);
  });

  it("filters non-object rows before the ranking UI consumes them", async () => {
    const rpc = {
      rpc: vi.fn(async () => ({
        data: [null, "broken", [], { rank_no: 1, display_name: "player one" }],
        error: null,
      })),
    };
    const client = new RankingClient({ rpcClient: rpc });

    await expect(client.getBestScores()).resolves.toEqual([
      { rank_no: 1, display_name: "player one" },
    ]);
  });

  it("rejects malformed results before making a verified submission", async () => {
    const fetch = vi.fn(
      async () =>
    ...
    await expect(
      client.submit("player one", null as unknown as RunResult, "run-token-1")
    ).rejects.toThrow("invalid run result");
  ...
  it("prepares a server-selected daily competition run", async () => {
    ...
    await expect(client.prepareCompetitionRun("player one")).resolves.toEqual({
      status: "ok",
      runToken: "competition-run-1",
      problemId: "AKERUN-07-V1",
      problemVersion: "V1",
