...
v2では、正しい最短経路でも発生する偽ゲート通過を問題ごとの基準値へ含め、不可避な通過がスコアへ二重計上されないようにした。この契約は、問題の再抽選、問題IDの差し替え、スコア式の改ざん、派生値の不一致、二重送信を防ぐ。クライアント側では、再開時に状態遷移と矛盾する端末チェックポイントも破棄する。ただし、悪意あるブラウザがプレイ結果の各メトリクスを自作することまでは証明しない。完全な入力トレース検証は、LockMechanismの決定的ルールをサーバー検証器へ移植する別フェーズで実装する。
