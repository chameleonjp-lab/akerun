-- Harden the first live contract migration.
--
-- The initial deployment used a three-argument prepare RPC while reception was
-- disabled. Replace it with the four-argument contract before enabling the
-- game: explicit problem selection is valid only with a completed same-
-- problem replay token. Extend the run retention window so an offline result
-- can still be retried from the device.
