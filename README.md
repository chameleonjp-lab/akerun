...
Supabase側の `akerun` は `submission_mode=verified` で登録しています。2026-08-29の読み取り確認では、公式問題20件、実行記録0件、ランキング記録0件で、`is_active=false` と `accepting_runs=false` を維持していました。本PRの結果ガードmigrationはまだ本番へ適用していないため、ランキング機能は引き続き停止します。通常プレイ自体は端末内記録として続けられます。サービスキーはEdge Functionのサーバー環境変数だけで使用し、ブラウザへ出しません。
